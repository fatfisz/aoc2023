...
pub fn main() !void {
    ...
    var initial_beams_buf: [1024]Beam = undefined;
    var initial_beams_len: usize = 0;
    for (0..height) |y| {
        initial_beams_buf[initial_beams_len] = .{ .x = 0, .y = @truncate(y), .direction = .right };
        initial_beams_buf[initial_beams_len + 1] = .{ .x = @truncate(width - 1), .y = @truncate(y), .direction = .left };
        initial_beams_len += 2;
    }
    for (0..width) |x| {
        initial_beams_buf[initial_beams_len] = .{ .x = @truncate(x), .y = 0, .direction = .bottom };
        initial_beams_buf[initial_beams_len + 1] = .{ .x = @truncate(x), .y = @truncate(height - 1), .direction = .top };
        initial_beams_len += 2;
    }

    var max_sum: Number = 0;
    for (initial_beams_buf[0..initial_beams_len]) |initial_beam| {
        var touch: [max_size][max_size]Touch = [_][max_size]Touch{[_]Touch{.{}} ** max_size} ** max_size;

        try queue.append(initial_beam);
